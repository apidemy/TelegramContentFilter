...
async def Kanger(c: Client, m: Message):
    await m.edit(text=f"Checking `{str(Config.SOURCE_CHAT_ID[0])}` ...")
    await asyncio.sleep(2)
    try:
        ForwardFromChat = await c.get_chat(chat_id=Config.SOURCE_CHAT_ID[0])
    ...
    for i in range(len(Config.DESTINATION_CHAT_ID)):
        await m.edit(text=f"Checking `{Config.DESTINATION_CHAT_ID[i]}` ...")
        await asyncio.sleep(2)
        try:
            ForwardToChat = await c.get_chat_member(chat_id=Config.DESTINATION_CHAT_ID[i], user_id=(await c.get_me()).id)
    ...
    async for message in c.iter_history(chat_id=Config.SOURCE_CHAT_ID[0], reverse=True):
